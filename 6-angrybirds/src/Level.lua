...
function Level:init()
    ...
    -- check if player collides obstacle or enemy: when it is true, not avalible to split player
    self.timesp = false
...
function Level:update(dt)
    -- if player collides, does not work splitting on 'space'
    if self.timesp then
    ...
    -- replace launch marker if original alien and his copies stopped moving
    if self.launchMarker.launched then
        if self.launchMarker.timesplit then
            for i = 1, 3, 1 do
                if self.kp[i] then
            ...
            if self.ap == 3 then
                ...
                -- restart depencies
               self.launchMarker.collides = false
               self.launchMarker.timesplit = false
               self.launchMarker.spacesplit = false
               self.timesp = false
               self.launchMarker = AlienLaunchMarker(self.world)
               self.kp = {
                [1] = true,
