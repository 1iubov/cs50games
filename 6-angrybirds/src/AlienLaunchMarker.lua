...
function AlienLaunchMarker:init(world)
    ...
    -- tap button
    self.spacesplit = false
    -- time after tap button and before addind 3 aliens
    self.timesplit = false
    self.collides = false
end

function AlienLaunchMarker:update(dt)
    -- if player did not collided (taken from level), we can split player
    if not self.collides and self.launched then
        if love.keyboard.wasPressed('space') then
            self.spacesplit = true
        end
    end
    ...
    if not self.launched then
        ...
        elseif love.mouse.wasReleased(1) and self.aiming then
            ...
            self.alien[1] = Alien(self.world, 'round', self.shiftedX, self.shiftedY, 'Player')
        ...
        if self.spacesplit and not self.timesplit then
            for i = 2, 3 do
                local xPos, yPos = self.alien[1].body:getPosition()
                self.shiftedX1 = xPos + math.random(-5, 5)
                self.shiftedY1 = yPos + math.random(-5, 5)
                -- spawn new alien in the world, passing in user data of player
                self.alien[i] = Alien(self.world, 'round', self.shiftedX1, self.shiftedY1, 'Player')

                -- apply the difference between current X,Y and base X,Y as launch vector impulse
                local xVel, yVel = self.alien[1].body:getLinearVelocity()
                xVel = xVel + math.random(-100, 100)
                yVel = yVel + math.random(-100, 100)
                self.alien[i].body:setLinearVelocity(xVel,yVel)

                -- make the alien pretty bouncy
                self.alien[i].fixture:setRestitution(0.4)
                self.alien[i].body:setAngularDamping(1)
            end
