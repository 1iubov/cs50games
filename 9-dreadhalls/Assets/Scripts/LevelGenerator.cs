...
public class LevelGenerator : MonoBehaviour {
	...
	// score, that means amount mazes before game over
	...
	// var to detect no more then 4 holes
	private int holes = 0;
	...
	void Start () {
		...
		for (int z = 0; z < mazeSize; z++) {
			for (int x = 0; x < mazeSize; x++) {
				...
				// it depends on counter level to feel level up
				if ((Random.Range(1,10) != 1) | (holes > 4)) {
