...
function LevelMaker.createMap(level)
    ...
    -- randomly choose the number of rows СТРОКИ
    -- local numRows = math.random(1, 5)
    local numRows = math.random(1, math.min(5, level + 1))

    -- randomly choose the number of columns, ensuring odd СТОЛБЦЫ
    -- local numCols = math.random(7, 13)
    local numCols = math.random(6, 11)
    ...
    -- key Bricks
    local keyBricks = level
    if level > 1 then math.random(0, level) end
    local lock = false
